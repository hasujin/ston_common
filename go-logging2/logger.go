...
func paulNow() time.Time {
	//fmt.Printf("location --> %v\n", location)
	if location == "" {
		return time.Now().UTC()
	}else{
		//location, err = time.LoadLocation("America/Los_Angeles")
		if location_, err := time.LoadLocation(location); err!=nil{
			log.Printf("WARNING : The given location(%s) is not supported, so its replaced with UTC\n", location)
			return time.Now().UTC()
		}else{
			return time.Now().In(location_)
		}
...
func MustGetLoggerPaul(module string, location_ string) *Logger {
	logger, err := GetLogger(module)
	if err != nil {
		panic("logger: " + module + ": " + err.Error())
	}
...
func (l *Logger) log(lvl Level, format *string, args ...interface{}) {
	...
	////////////////////////////////////////////////////////////////////////////////
	//t:= time.Now().UTC().In(time.FixedZone("", offsetHrs*60*60))
	//t:= time.Now().UTC().In(time.FixedZone("", offsetHrs*60*60))
	//t= t.In(time.FixedZone("KST", 9*60*60)) // --> Seoul
	////////////////////////////////////////////////////////////////////////////////

	record := &Record{
		ID:     atomic.AddUint64(&sequenceNo, 1),
		Time:   timeNow(), //paulNow(),
		//Time: t, 	// customized by paul
		Module: l.Module,
		Level:  lvl,
		fmt:    format,
		Args:   args,
